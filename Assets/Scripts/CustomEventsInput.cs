...
public class CustomEventsInput : MonoBehaviour
{
    ...
    void Update()
    {
        if (timer <= 0)
        {
            ...
            if (Input.GetAxis("Dpad Left Vertical") > deadZone) // move up
            {
                currentAxis.moveDir = MoveDirection.Up;
                ExecuteEvents.Execute(currentButton, currentAxis, ExecuteEvents.moveHandler);
            }
            else if (Input.GetAxis("Dpad Left Vertical") < -deadZone) // move down
            {
                currentAxis.moveDir = MoveDirection.Down;
                ExecuteEvents.Execute(currentButton, currentAxis, ExecuteEvents.moveHandler);
            }
            else if (Input.GetAxis("Dpad Left Horizontal") > deadZone) // move right
            {
                currentAxis.moveDir = MoveDirection.Right;
                ExecuteEvents.Execute(currentButton, currentAxis, ExecuteEvents.moveHandler);
            }
            else if (Input.GetAxis("Dpad Left Horizontal") < -deadZone) // move left
            {
